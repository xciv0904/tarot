...
| 2026-08-12 | 主題答案的句型要從題目問的東西講起 | 「壓力大時我會出現哪些反應？」原本答「壓力多半在無法⋯時累積」——講的是來源不是反應；detail 標籤寫「壓力來臨時的反應」，內容卻是「壓力來源是缺少⋯」。改成 phrase 用 {risk}（那本來就是反應）、summary 講條件、detail 講節奏與被忽略的部分 |
| 2026-08-12 | 分項內文不得把自己的標籤再唸一次 | 標籤「壓力來臨時的反應」配內文「壓力來臨時的反應會偏向⋯」。原本的 ECHO_RE 只認「X是／宜／為」，用「會」就繞過了。已補上開頭四字與標籤的重疊比對，並改寫兩個會踩到的句型 |
| 2026-08-12 | 牌組選擇不再放在原生 <details> 裡 | render() 整段重畫 innerHTML，<details> 的開合只存在 DOM，所以裡面任何觸發 render() 的按鈕一按就把整區收起來——使用者形容成「跳回上一頁，要再點一次」。牌組只有兩個選項，改成常駐顯示；差別說明改用 state.tlGuideOpen |
| 2026-08-12 | Golden 7f35b9a867161134 → 8b943725f183a9ca | 壓力題與學習題句型改寫、海王星 need 縮短為 12 字（與其他行星同量級） |
| 2026-08-12 | 結果頁移除「想再看懂這次的牌」整區 | 使用者回報「太多了⋯而且同一頁面出現兩次」。那一區的四個項目全部是把頁面上方既有函式的輸出再展開一次：detail-core = overallReading()（＝整副牌的走向）、detail-positions = 每張牌的牌位解讀、detail-tension 已嵌在 overallReading 裡。要看懂這次的牌，答案本來就在上面 |
| 2026-08-12 | 有「直接回答」面板時，整副牌的走向預設收合 | 兩者是同一批內容的兩種寫法（條列 vs 連續敘述）。都攤開就是同一頁兩份幾乎一樣的長段落。收合後仍可「讀完整敘述」，歷史紀錄與複製給 AI 不受影響 |
| 2026-08-12 | 語氣只講一次；牌位名稱含「的」時改用「在」 | directQuestionLead 退回「這件事＋語氣」時，開頭又寫「這組牌＋同一個語氣」；readingCardIdentity 一律串「的」，於是「過去的影響的『戒指』」。兩者都加進 reading-copy-quality，對修正前的 app.js 分別抓到 54 例與 4 個牌位 |
| 2026-08-12 | 牌面圖片改用獨立的 mystic-assets-v1 快取，不隨程式版本清空 | 圖片原本跟程式共用版本化快取，sw 的 activate 會刪掉所有非當前版本的快取——這一輪我連續把 mystic-v61 推到 v66，等於把 228 張牌圖清空六次。清空後整批重抓時，assets 分支又沒有 catch，任何一張逾時就變成網路錯誤破圖。圖片檔名本身就是內容，沒有理由跟著程式改版失效 |
| 2026-08-12 | assets 抓取失敗要重試並退回舊快取，最後才給 Response.error() | 原本  沒有 catch，rejected promise 進到 respondWith 就是破圖 |
| 2026-08-12 | 牌圖 onerror 退回牌名方塊 | 不管破圖原因是網路、快取還是擋圖擴充套件，畫面上都不該出現破圖；alt 本來就帶著牌名 |
| 2026-08-12 | 綜合觀察的核心結論先給內容，最後才說這兩層怎麼用 | 原本開頭是「兩邊都沒有特別強烈的傾向，方向一致地保持中性」——那是在講兩個來源合不合，不是回答問題，而且面板標題已經說過同一件事。使用者看完仍然不知道該怎麼辦 |
| 2026-08-12 | 延伸問題區移到整份解讀的最後 | 原本緊接在牌卡下方，也就是使用者還沒讀到任何解讀內容之前就先被問「要不要換一個問題」；那一區的三張卡片還會把真正的解讀往下推。測試改成比對渲染後的位置（在牌典連結、整副牌的走向、解讀語氣之後，重新提問之前），而不是只檢查字串存在 |
| 2026-08-12 | 破壞性操作全面改為畫面內兩段式確認 | 四處原本是 try { if (!confirm(...)) return; } catch (e) {}。confirm 被抑制時回傳 false 還算安全，但丟例外時 catch 會吞掉、程式繼續往下走——確認框沒出現，資料照樣被刪。統一走 armDestructive()／renderArmedConfirm() |
| 2026-08-12 | 加入全域錯誤邊界 | 畫面是 view.innerHTML = render() 一次寫進去的，render 丟例外＝空白畫面且毫無訊息。只攔有 e.error 的事件，資源載入失敗各自有降級處理 |
| 2026-08-12 | 加入匯出／匯入備份 | localStorage 是唯一的家，原本沒有出口而清除按鈕不可復原。還原只接受 APP_STORAGE_KEYS 白名單內的 key 與字串值，並檢查 format 欄位 |
| 2026-08-12 | 加入 meta 版 CSP | 純靜態站沒有後端可送 header。script-src 必須保留 unsafe-inline（全站互動是 onclick 屬性），真正的防線是所有插入畫面的字串都經過 esc() |
| 2026-08-12 | 主問題從第三步移到第二步，牌陣推薦改由主問題決定 | 第一步明講「系統會依照內容推薦合適的牌陣」，但第二步一直只讀 RECOMMENDATIONS[category]——taxonomy 的 44 個 primary 全部寫好了 spreads，沒有一行程式讀它。順序本來也是反的：先挑工具再說要做什麼。現在換一個主問題，下面的牌陣清單當場跟著換，因果在同一屏 |
| 2026-08-12 | 雷諾曼維持自己的推薦表 | taxonomy 的 spreads 是照塔羅牌陣命名的，硬套會推薦到不存在的牌陣 |
| 2026-08-12 | 結果頁：翻牌留在牌卡旁，語氣與複製移到解讀之後 | 「直接回答」原本被語氣選擇與按鈕列擋在後面。翻牌／顯示牌義是讀解讀之前要用的，語氣與複製是讀完才會用的 |
| 2026-08-12 | 首頁不再要求新手先讀完牌典 | 原句「初次接觸塔羅嗎？建議先到牌典認識 22 張牌，再回來開始占卜」等於要新手先讀完才有資格用 |
| 2026-08-12 | 牌陣推薦是「排序」不是「篩選」 | 上一版只回傳主問題那組，結果單身桃花、暗戀、三張牌・解析、時間軸在某些分類整個從畫面上消失——推薦把選項刪掉了。現在分兩組：「更貼近這一題」在前、「這個領域也常用」在後，一個都不拿掉。測試對每個 primary 檢查該分類原有的牌陣是否都還在 |
| 2026-08-12 | 單身桃花／暗戀牌陣補進感情的主問題 | peach 補進三個 futurePerson 的主問題，crush 補進兩個 specificPerson 的主問題——它們本來就是那幾題最貼切的牌陣 |
| 2026-08-12 | 幸運色與幸運數字改回當日主星，並與命主星分開標示 | 掛在當日主星上被回報「不管哪張星盤都一樣」，改掛命主星後被回報「每天都一樣」。兩次都對——那是兩件事被塞進同一格。沒有「個人專屬又每天不同」的古典幸運色，與其編一個，不如把命主星（你的、不變）與今日主星（曆法事實、每天變、全球共通）各自標清楚。幸運時段才是兩者交會處 |
| 2026-08-12 | 牌陣本身也帶前提（SPREAD_ASSUMPTIONS） | 「暗戀牌陣」的整個前提就是有一個特定對象、「單身桃花牌陣」是還沒有對象。以前只有 primary 會過濾面向，所以選了暗戀牌陣卻還沒選主問題的人，看到的是一整排通用面向，跟選單身牌陣一模一樣。只有前提明確的牌陣列進表裡——三張牌・時間流、凱爾特十字這類通用版面不帶前提，不憑空幫使用者假設處境 |
| 2026-08-12 | 因牌陣才出現的面向排在通用面向之前 | 只預覽前 5 個，不排序的話選了暗戀牌陣第一眼看到的仍然是「我容易被他什麼特質吸引」這種任何狀況都適用的項目 |
| 2026-08-12 | 選了主問題之後，主問題說了算 | 牌陣前提只在「還沒選主問題」時生效，不得覆蓋 primary 的 assumptions |
