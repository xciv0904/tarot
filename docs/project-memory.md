...
| 2026-08-03 | 今日一牌的種子從「日期」改為「日期＋裝置碼」 | 原本全世界同一天拿到同一張牌。放在「抽牌」的語境裡，使用者理所當然以為那是自己抽到的。裝置碼是純本機亂數，不含個人資訊；無法寫入 localStorage 時退回日期種子（牌仍一天固定，只是可能與其他同樣無法寫入的裝置相同，這比每次重整換一張好） |
| 2026-08-03 | 卡框高度改由 `CARD_ART_RATIO` 推導，不得寫死 | 牌面圖一律 0.6 比例（420×700 / 縮圖 300×500）。先前今日一牌框高寫死 262px，扣掉底部名稱區後圖片可用區變成 0.93 比例，`object-fit:contain` 以高度縮放，左右各留 28px 白邊 |
