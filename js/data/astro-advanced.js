...
function buildAstroDataPackText(chart, unknownTime) {
  ...
  L.push('【交給 AI 的任務】');
  L.push('開頭先寫 3 個使用者在日常生活中容易立刻認出的具體表現，每點一至兩句；不要一開始就宣判人生主軸，也不要先講分析方法或限制。');
  L.push('接著整理 3–5 個會反覆出現的人生模式，閱讀順序由近到深：日常或工作／學習 → 關係互動 → 壓力與決策 → 最深層的成長主題。');
  L.push('每個模式請寫成生活片段：常在哪種情況被觸發 → 通常怎麼反應 → 這樣做的優勢與代價 → 更好用的做法。不要讓每段都使用完全相同的小標與句型。');
  L.push('重要結論須由至少兩項落點或相位共同支持；依據放在該段最後的簡短括號內即可，不要讓占星術語打斷閱讀。');
  L.push('遇到相反訊號時請判斷哪一邊較強，再說明另一面通常在什麼情境出現。不要反覆使用「可能、也許、不一定、僅供參考」等模糊語氣。只有資料限制確實會改變結論時才提醒一次。');
  L.push('全部模式說明完之後，才用一小段「核心主軸」收束共同脈絡。不預測必然事件，也不要用「你很矛盾」帶過。');
  L.push('最後請給 3 項與前文直接對應、這週就能執行的具體建議，不必再反問使用者。');
