...
function buildAstroDataPackText(chart, unknownTime) {
  ...
  L.push('每一類只寫一個 90–140 字的段落，優先回答該類核心問題，並自然涵蓋前述自我檢查中的至少三項；不要拆成固定小標，也不要只列人格形容詞。');
  L.push('三、【三個跨分類模式】：找出全盤最明顯的三條共同脈絡。每個模式用 100–150 字完整呈現「觸發情境 → 慣性反應 → 實際代價 → 替代做法」，必須連結至少兩個生活分類，並解釋不同領域為何出現同一反應；只寫前面尚未說清楚的關聯，不得換句話重複八類內容。');
