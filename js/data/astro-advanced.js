...
function buildAstroDataPackText(chart, unknownTime) {
  ...
  L.push('【交給 AI 的任務】');
  L.push('請先直接給出一句清楚的整體結論，再整理 3–5 個最有辨識度、會反覆出現的人生模式。不要先講分析方法或限制。');
  L.push('每個模式請寫成使用者能認出的生活片段：常在哪種情況被觸發 → 通常怎麼反應 → 這樣做的優勢與代價 → 更好用的做法。請涵蓋關係、工作／學習與壓力下的選擇。');
  ...
  L.push('不預測必然事件，也不要用「你很矛盾」帶過。結尾請給 3 項與前文直接對應、這週就能執行的具體建議，不必再反問使用者。');
