...
function buildAstroDataPackText(chart, unknownTime) {
  ...
  L.push('一、【快速認識你】：用 80–120 字寫出最容易在日常生活中被本人認出的整體輪廓，不要一開始就宣判人生主軸。');
  ...
  L.push('7. 核心人生課題');
  L.push('8. 現在可以怎麼運用');
  L.push('每一類都要包含：日常中的具體表現、優勢、過度使用時的代價，以及更成熟實際的做法。內容要寫成生活片段，不要只列人格形容詞。');
  L.push('三、【三個跨分類模式】：找出全盤最明顯的三條共同脈絡。每個模式必須連結至少兩個生活分類，並寫清楚觸發情境 → 慣性反應 → 真正需求 → 代價 → 可替代的做法，避免重複前面八類的原句。');
  L.push('重要結論須由至少兩項落點或相位共同支持；依據放在相關段落最後的簡短括號內即可，不要讓占星術語打斷閱讀。');
  L.push('遇到相反訊號時請判斷哪一邊較強，再說明另一面通常在什麼情境出現。不要反覆使用「可能、也許、不一定、僅供參考」等模糊語氣。只有資料限制確實會改變結論時才提醒一次。');
  L.push('不預測必然事件，也不要用「你很矛盾」帶過。');
  L.push('四、【本週行動】：最後給 3 項與前文直接對應、這週就能執行且能確認是否完成的具體行動，不必再反問使用者。');
