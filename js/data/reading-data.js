...
var FOCUS_AREA_REGISTRY = {
  ...
  /* ---- 健康：定位在壓力、節奏與自我照顧，不做醫療判斷 ---- */
  'ht-drain': { label: '目前最容易消耗我的因素', semanticFocus: 'energy_drain' },
  'ht-habit': { label: '哪個生活習慣最需要調整', semanticFocus: 'habit_adjust' },
  'ht-rest': { label: '我是不是忽略了休息', semanticFocus: 'rest_neglect' },
  'ht-stress-first': { label: '哪種壓力最值得先處理', semanticFocus: 'stress_priority' },
  'ht-stabilise': { label: '如何讓生活恢復穩定', semanticFocus: 'life_stabilise' },
  'ht-sleep': { label: '睡眠品質受什麼影響', semanticFocus: 'sleep_factor' },
  'ht-eat': { label: '飲食節奏哪裡失衡', semanticFocus: 'eating_rhythm' },
  'ht-move': { label: '身體活動量夠不夠', semanticFocus: 'activity_level' },
  'ht-mood': { label: '情緒狀態最近的走向', semanticFocus: 'mood_trend' },
  'ht-anxiety': { label: '健康焦慮從哪裡來', semanticFocus: 'health_anxiety' },
  'ht-seek-help': { label: '求助時卡住我的是什麼', semanticFocus: 'help_seeking_block' },
  'ht-selfcare': { label: '最適合我的自我照顧方式', semanticFocus: 'self_care_style' },

  /* ---- 財運 ---- */
  'wl-spend-risk': { label: '最大的支出風險', semanticFocus: 'spend_risk' },
  'wl-income-stable': { label: '收入的穩定性', semanticFocus: 'income_stability' },
  'wl-hidden-cost': { label: '容易被忽略的花費', semanticFocus: 'hidden_cost' },
  'wl-save-block': { label: '存不下錢的阻礙', semanticFocus: 'saving_block' },
  'wl-next-tidy': { label: '下一步最值得整理什麼', semanticFocus: 'next_cleanup' },
  'wl-impulse': { label: '衝動消費的觸發點', semanticFocus: 'impulse_trigger' },
  'wl-value': { label: '我真正願意花錢的地方', semanticFocus: 'spending_value' },
  'wl-risk-tolerance': { label: '我能承受多少風險', semanticFocus: 'risk_tolerance' },
  'wl-allocation': { label: '資金配置的習慣', semanticFocus: 'allocation_habit' },
  'wl-info-gap': { label: '決策時缺少哪些資訊', semanticFocus: 'information_gap' },
  'wl-blindspot': { label: '財務決策的盲點', semanticFocus: 'financial_blindspot' },
  'wl-income-source': { label: '收入可能從哪裡增加', semanticFocus: 'income_source' },
  'wl-pressure': { label: '財務壓力真正的來源', semanticFocus: 'financial_pressure' },
  'wl-longplan': { label: '長期資源該怎麼安排', semanticFocus: 'long_term_resource' },

  /* ---- 人際 ---- */
  'sc-drift-why': { label: '關係改變的原因', semanticFocus: 'drift_cause', requires: { specificPerson: true } },
  'sc-their-stance': { label: '對方目前的互動態度', semanticFocus: 'their_attitude', requires: { specificPerson: true } },
  'sc-my-miss': { label: '我可能忽略了什麼', semanticFocus: 'own_oversight' },
  'sc-initiate': { label: '是否需要我主動', semanticFocus: 'initiative', requires: { specificPerson: true } },
  'sc-adjust': { label: '這段關係適合怎麼調整', semanticFocus: 'relation_adjust', requires: { specificPerson: true } },
  'sc-new-where': { label: '比較容易在哪裡認識新朋友', semanticFocus: 'new_social_context' },
  'sc-new-style': { label: '我適合哪種交友方式', semanticFocus: 'social_style' },
  'sc-new-open': { label: '我對新關係的開放程度', semanticFocus: 'openness' },
  'sc-group-role': { label: '我在團體中的角色', semanticFocus: 'group_role' },
  'sc-group-fit': { label: '我和這個群體合不合', semanticFocus: 'group_fit' },
  'sc-conflict-core': { label: '衝突的核心是什麼', semanticFocus: 'conflict_core' },
  'sc-conflict-fix': { label: '可以怎麼修復', semanticFocus: 'conflict_repair' },
  'sc-boundary': { label: '界線該畫在哪裡', semanticFocus: 'social_boundary' },
  'sc-trust': { label: '信任是怎麼被影響的', semanticFocus: 'trust_dynamic' },
  'sc-ally': { label: '誰可能是真正的助力', semanticFocus: 'ally' },

  /* ---- 學業 ---- */
  'st-weak': { label: '最需要補強的地方', semanticFocus: 'weak_area', requires: { examOrGoal: true } },
  'st-time': { label: '時間分配的問題', semanticFocus: 'time_allocation' },
  'st-focus': { label: '注意力容易被什麼打斷', semanticFocus: 'focus_disruption' },
  'st-anxiety': { label: '考試焦慮的來源', semanticFocus: 'exam_anxiety', requires: { examOrGoal: true } },
  'st-strategy': { label: '準備策略要怎麼調整', semanticFocus: 'study_strategy', requires: { examOrGoal: true } },
  'st-losepoint': { label: '容易失分的習慣', semanticFocus: 'error_pattern', requires: { examOrGoal: true } },
  'st-method': { label: '適合我的學習方式', semanticFocus: 'learning_style' },
  'st-retain': { label: '記不住的原因', semanticFocus: 'retention' },
  'st-motivation': { label: '動力從哪裡來', semanticFocus: 'motivation' },
  'st-major-fit': { label: '這個方向和我的適配度', semanticFocus: 'field_fit' },
  'st-major-future': { label: '這條路的發展空間', semanticFocus: 'field_prospect' },
  'st-continue': { label: '這個學習計畫值不值得繼續', semanticFocus: 'continue_worth' },
  'st-progress': { label: '進度卡在哪一段', semanticFocus: 'progress_block' },
  'st-pressure': { label: '學習壓力怎麼調節', semanticFocus: 'study_pressure' },

  /* ---- 決策 ---- */
  'dc-priority': { label: '我真正優先考慮的是什麼', semanticFocus: 'choice_priority' },
  'dc-a-chance': { label: '選 A 的機會', semanticFocus: 'a_opportunity', requires: { twoOptions: true } },
  'dc-a-cost': { label: '選 A 的代價', semanticFocus: 'a_cost', requires: { twoOptions: true } },
  'dc-b-chance': { label: '選 B 的機會', semanticFocus: 'b_opportunity', requires: { twoOptions: true } },
  'dc-b-cost': { label: '選 B 的代價', semanticFocus: 'b_cost', requires: { twoOptions: true } },
  'dc-overlook': { label: '最容易被我忽略的因素', semanticFocus: 'overlooked_factor' },
  'dc-third': { label: '有沒有第三種做法', semanticFocus: 'third_option' },
  'dc-do-why': { label: '想做的真正原因', semanticFocus: 'motive_check' },
  'dc-do-cost': { label: '做了要付出什麼', semanticFocus: 'action_cost' },
  'dc-not-do': { label: '不做會失去什麼', semanticFocus: 'inaction_cost' },
  'dc-now-ready': { label: '現在的條件夠不夠', semanticFocus: 'readiness' },
  'dc-wait-gain': { label: '等待能換到什麼', semanticFocus: 'waiting_value' },
  'dc-wait-risk': { label: '等待的風險', semanticFocus: 'waiting_risk' },
  'dc-stay-why': { label: '留下來的理由還成立嗎', semanticFocus: 'stay_reason' },
  'dc-leave-ready': { label: '離開需要先準備什麼', semanticFocus: 'leaving_prep' },
  'dc-accept-fit': { label: '這個機會和我的契合度', semanticFocus: 'offer_fit', requires: { specificOffer: true } },
  'dc-accept-risk': { label: '接受後最大的風險', semanticFocus: 'accept_risk', requires: { specificOffer: true } },
  'dc-unclear-signal': { label: '目前手上有哪些線索', semanticFocus: 'available_signal' },
  'dc-unclear-need': { label: '我現在最需要什麼', semanticFocus: 'current_need' },

  /* ---- 綜合 ---- */
  'gn-mood': { label: '情緒狀態', semanticFocus: 'overall_mood' },
  'gn-relation': { label: '關係層面', semanticFocus: 'overall_relation' },
  'gn-work': { label: '工作或學習', semanticFocus: 'overall_work' },
  'gn-money': { label: '金錢與資源', semanticFocus: 'overall_money' },
  'gn-action': { label: '行動與推進', semanticFocus: 'overall_action' },
  'gn-rest': { label: '休息與恢復', semanticFocus: 'overall_rest' },
  'gn-theme': { label: '這階段的主軸', semanticFocus: 'stage_theme' },
  'gn-priority': { label: '最需要先處理的一件事', semanticFocus: 'top_priority' },
  'gn-blindspot': { label: '目前的盲點', semanticFocus: 'current_blindspot' },
  'gn-next': { label: '下一步的方向', semanticFocus: 'next_step' },
  'gn-support': { label: '身邊有什麼支持', semanticFocus: 'available_support' },
  'gn-drain': { label: '什麼正在消耗我', semanticFocus: 'current_drain' },
};
  ...
  love: {
    ...
    primaries: [
      { id: 'lv-future', legacyKey: 'partner-type', label: '未來可能遇到什麼類型的人', intent: 'future_partner_profile',
        ...
        spreads: ['peach', 'three-time', 'single', 'celtic'],
      ...
      { id: 'lv-meet', legacyKey: 'meet-scene', label: '可能在哪種場合認識', intent: 'meeting_context',
        assumptions: { futurePerson: true },
        spreads: ['peach', 'three-time', 'single', 'horseshoe'],
      ...
      { id: 'lv-target', legacyKey: 'crush', label: '對方現在怎麼看我', intent: 'partner_current_view',
        ...
        spreads: ['crush', 'relationship', 'crosslove', 'three-time'],
...
function taxonomyPrimaryForSubtopic(catId, subtopicKey) {
  ...
  /* 有舊題目的分類靠 legacyKey 對應；沒有舊題目的分類（決策）直接用 taxonomy id。 */
  return prs.filter(function (p) { return p.legacyKey === subtopicKey; })[0]
      || prs.filter(function (p) { return p.id === subtopicKey; })[0] || null;
}
...
/* 其餘六類的遷移。health 與 wealth 帶 safetyRules——塔羅不做醫療判斷、也不承諾報酬。 */
QUESTION_TAXONOMY.health = {
  categoryId: 'health', label: '健康',
  placeholder: '例如：我最近一直很累，但說不上來是哪裡出問題。',
  safetyRules: ['塔羅不能取代醫療評估；若有症狀或健康疑慮，請諮詢合格醫療專業。'],
  primaries: [
    { id: 'ht-status', legacyKey: 'body-lifestyle', label: '最近身心狀態需要注意什麼', intent: 'wellbeing_status',
      assumptions: {}, spreads: ['three-mbs', 'three-time', 'single'],
      focus: ['ht-drain', 'ht-stress-first', 'ht-rest', 'ht-habit', 'ht-stabilise', 'ht-mood'],
      placeholder: '例如：我最近一直很累，生活中最需要先調整什麼？',
      examples: { base: ['我最近一直覺得疲憊，但說不出是哪裡的問題。'],
        byFocus: { 'ht-rest': ['我明明有睡，但起床還是覺得沒有恢復。'],
                   'ht-stress-first': ['我同時被好幾件事壓著，不知道該先處理哪一個。'] } } },
    { id: 'ht-rhythm', legacyKey: 'daily-balance', label: '生活節奏該怎麼調整', intent: 'daily_rhythm',
      assumptions: {}, spreads: ['three-mbs', 'three-time', 'horseshoe'],
      focus: ['ht-sleep', 'ht-eat', 'ht-move', 'ht-rest', 'ht-habit', 'ht-stabilise'],
      placeholder: '例如：我的作息很亂，想知道從哪裡開始調整比較實際。',
      examples: { base: ['我的作息很亂，想知道從哪裡開始調整。'],
        byFocus: { 'ht-sleep': ['我常常很晚才睡，隔天整天都提不起勁。'],
                   'ht-move': ['我知道該運動，但一直沒有真的開始。'] } } },
    { id: 'ht-care', legacyKey: 'self-care-symbolic', label: '我適合什麼樣的自我照顧', intent: 'self_care_style',
      assumptions: {}, spreads: ['three-mbs', 'single', 'celtic'],
      focus: ['ht-selfcare', 'ht-anxiety', 'ht-seek-help', 'ht-mood', 'ht-drain'],
      placeholder: '例如：我知道要照顧自己，但不確定什麼方式對我有效。',
      examples: { base: ['我知道要照顧自己，但不確定什麼方式對我有效。'],
        byFocus: { 'ht-seek-help': ['我一直沒去看醫生，好像有什麼在卡著我。'],
                   'ht-anxiety': ['我最近很容易擔心自己的身體。'] } } },
  ],
};
QUESTION_TAXONOMY.wealth = {
  categoryId: 'wealth', label: '財運',
  placeholder: '例如：最近收入沒有變，但支出增加很多，我想知道最需要整理什麼。',
  safetyRules: ['塔羅不提供投資標的、報酬率或買賣建議；財務決策請自行評估或諮詢專業。'],
  primaries: [
    { id: 'wl-status', legacyKey: 'cashflow-risk', label: '最近財務最需要注意什麼', intent: 'financial_status',
      assumptions: {}, spreads: ['three-time', 'horseshoe', 'single'],
      focus: ['wl-spend-risk', 'wl-income-stable', 'wl-hidden-cost', 'wl-save-block', 'wl-next-tidy', 'wl-pressure'],
      placeholder: '例如：收入沒變但支出變多，我想知道問題出在哪。',
      examples: { base: ['收入沒變但支出變多，我想知道問題出在哪。'],
        byFocus: { 'wl-hidden-cost': ['我不太清楚錢到底花去哪裡了。'],
                   'wl-save-block': ['我每個月都想存錢，但總是存不下來。'] } } },
    { id: 'wl-attitude', legacyKey: 'risk-approach', label: '我面對錢的態度需要調整什麼', intent: 'money_attitude',
      assumptions: {}, spreads: ['three-mbs', 'three-time', 'celtic'],
      focus: ['wl-risk-tolerance', 'wl-impulse', 'wl-value', 'wl-allocation', 'wl-blindspot', 'wl-info-gap'],
      placeholder: '例如：我常常衝動買東西，事後又後悔。',
      examples: { base: ['我常常衝動消費，事後又後悔。'],
        byFocus: { 'wl-risk-tolerance': ['我不確定自己能承受多少風險。'],
                   'wl-value': ['我想知道自己真正願意把錢花在什麼上面。'] } } },
    { id: 'wl-source', legacyKey: 'opportunity-source', label: '收入可能從哪裡增加', intent: 'income_growth',
      assumptions: {}, spreads: ['three-time', 'horseshoe', 'celtic'],
      focus: ['wl-income-source', 'wl-income-stable', 'wl-allocation', 'wl-info-gap', 'wl-longplan'],
      placeholder: '例如：我想增加收入，但不知道從哪個方向著手。',
      examples: { base: ['我想增加收入，但不知道從哪裡開始。'],
        byFocus: { 'wl-income-source': ['我在考慮做點副業，但還沒有頭緒。'] } } },
    { id: 'wl-plan', legacyKey: 'money-pattern', label: '長期的錢該怎麼安排', intent: 'long_term_finance',
      assumptions: {}, spreads: ['timeline', 'three-time', 'celtic'],
      focus: ['wl-longplan', 'wl-allocation', 'wl-pressure', 'wl-save-block', 'wl-value'],
      placeholder: '例如：我想開始做長期的財務規劃，但不知道優先順序。',
      examples: { base: ['我想做長期規劃，但不知道優先順序。'],
        byFocus: { 'wl-longplan': ['我想知道未來幾年該把資源放在哪裡。'] } } },
  ],
};
QUESTION_TAXONOMY.social = {
  categoryId: 'social', label: '人際',
  placeholder: '例如：一位朋友最近明顯變冷淡，我不知道是不是哪裡出了問題。',
  primaries: [
    { id: 'sc-drift', legacyKey: 'ally-conflict', label: '為什麼某個關係最近變疏遠', intent: 'relationship_drift',
      assumptions: { specificPerson: true }, spreads: ['crosslove', 'relationship', 'three-time'],
      focus: ['sc-drift-why', 'sc-their-stance', 'sc-my-miss', 'sc-initiate', 'sc-adjust', 'sc-trust'],
      placeholder: '例如：一位朋友最近明顯變冷淡，我不知道發生什麼事。',
      examples: { base: ['一位朋友最近明顯變冷淡。'],
        byFocus: { 'sc-initiate': ['我不知道該不該主動聯絡他。'],
                   'sc-my-miss': ['我懷疑是不是我哪裡做得不好。'] } } },
    { id: 'sc-new', legacyKey: 'attract-type', label: '怎麼認識更多合得來的人', intent: 'new_connections',
      assumptions: {}, spreads: ['three-time', 'horseshoe', 'single'],
      focus: ['sc-new-where', 'sc-new-style', 'sc-new-open', 'sc-group-fit', 'sc-ally'],
      placeholder: '例如：我想擴展生活圈，但不知道從哪裡開始。',
      examples: { base: ['我想認識更多人，但不知道從哪裡開始。'],
        byFocus: { 'sc-new-where': ['我的生活圈很固定，幾乎遇不到新的人。'] } } },
    { id: 'sc-style', legacyKey: 'interpersonal-style', label: '我的相處方式有什麼盲點', intent: 'social_pattern',
      assumptions: {}, spreads: ['three-mbs', 'celtic', 'three-time'],
      focus: ['sc-my-miss', 'sc-boundary', 'sc-trust', 'sc-group-role', 'sc-new-style'],
      placeholder: '例如：我常常在關係裡覺得累，但說不出原因。',
      examples: { base: ['我常常在人際關係裡覺得累。'],
        byFocus: { 'sc-boundary': ['我很難拒絕別人，事後又覺得委屈。'] } } },
    { id: 'sc-group', legacyKey: 'social-need-pattern', label: '我在團體裡的位置該怎麼調整', intent: 'group_dynamic',
      assumptions: {}, spreads: ['three-time', 'crosslove', 'celtic'],
      focus: ['sc-group-role', 'sc-group-fit', 'sc-conflict-core', 'sc-conflict-fix', 'sc-ally'],
      placeholder: '例如：我在團體裡總覺得格格不入。',
      examples: { base: ['我在團體裡總覺得格格不入。'],
        byFocus: { 'sc-conflict-core': ['最近團體裡有些摩擦，我不確定核心是什麼。'] } } },
  ],
};
QUESTION_TAXONOMY.study = {
  categoryId: 'study', label: '學業',
  placeholder: '例如：考試剩兩個月，但我一直無法穩定安排讀書時間。',
  primaries: [
    { id: 'st-exam', legacyKey: 'exam-application', label: '目前考試準備最大的問題是什麼', intent: 'exam_preparation',
      assumptions: { examOrGoal: true }, spreads: ['three-issue', 'horseshoe', 'three-time'],
      focus: ['st-weak', 'st-time', 'st-focus', 'st-anxiety', 'st-strategy', 'st-losepoint'],
      placeholder: '例如：考試剩兩個月，但我一直無法穩定安排讀書時間。',
      examples: { base: ['考試剩不到兩個月，我還沒進入狀況。'],
        byFocus: { 'st-anxiety': ['我一想到考試就緊張，反而讀不下去。'],
                   'st-time': ['我每天都有讀，但進度一直落後。'] } } },
    { id: 'st-how', legacyKey: 'focus-execution', label: '我的學習方式需要調整什麼', intent: 'learning_method',
      assumptions: {}, spreads: ['three-mbs', 'three-issue', 'celtic'],
      focus: ['st-method', 'st-focus', 'st-retain', 'st-time', 'st-motivation'],
      placeholder: '例如：我讀了很多但都記不住。',
      examples: { base: ['我讀了很多，但好像都沒有真的吸收。'],
        byFocus: { 'st-retain': ['我當下看懂了，過幾天又忘光。'],
                   'st-focus': ['我很容易分心，一坐下就想滑手機。'] } } },
    { id: 'st-field', legacyKey: 'major-fit', label: '這個科系或方向適合我嗎', intent: 'field_fit',
      assumptions: {}, spreads: ['fork', 'three-time', 'celtic'],
      focus: ['st-major-fit', 'st-major-future', 'st-motivation', 'st-continue', 'st-method'],
      placeholder: '例如：我不確定現在念的方向是不是真的適合我。',
      examples: { base: ['我不確定現在念的方向適不適合我。'],
        byFocus: { 'st-major-future': ['我想知道這條路之後的發展空間。'] } } },
    { id: 'st-progress', legacyKey: 'learning-trait', label: '學習進度卡住了該怎麼辦', intent: 'progress_block',
      assumptions: {}, spreads: ['three-issue', 'horseshoe', 'timeline'],
      focus: ['st-progress', 'st-pressure', 'st-motivation', 'st-time', 'st-continue'],
      placeholder: '例如：我的研究進度停在同一個地方很久了。',
      examples: { base: ['我的進度停在同一個地方很久了。'],
        byFocus: { 'st-pressure': ['壓力大到我開始逃避打開檔案。'] } } },
  ],
};
QUESTION_TAXONOMY.decision = {
  categoryId: 'decision', label: '決策',
  placeholder: '例如：我正在 A 和 B 之間猶豫，A 比較穩定，B 的發展性更高。',
  primaries: [
    { id: 'dc-ab', legacyKey: null, label: 'A 和 B 我該選哪一個', intent: 'binary_choice',
      assumptions: { twoOptions: true }, spreads: ['fork', 'crosslove', 'celtic'],
      focus: ['dc-a-chance', 'dc-a-cost', 'dc-b-chance', 'dc-b-cost', 'dc-priority', 'dc-overlook'],
      placeholder: '例如：A 比較穩定，B 的發展性更高，我很難取捨。',
      examples: { base: ['我在兩個選項之間猶豫，各有優缺點。'],
        byFocus: { 'dc-priority': ['我不確定自己現在最該優先考慮什麼。'] } } },
    { id: 'dc-do', legacyKey: null, label: '這件事我到底要不要做', intent: 'act_or_not',
      assumptions: {}, spreads: ['yesno', 'three-issue', 'three-time'],
      focus: ['dc-do-why', 'dc-do-cost', 'dc-not-do', 'dc-overlook', 'dc-third'],
      placeholder: '例如：我一直想做一件事，但每次都在最後退縮。',
      examples: { base: ['我一直想做這件事，但每次都退縮。'],
        byFocus: { 'dc-not-do': ['我想知道如果不做，我會失去什麼。'] } } },
    { id: 'dc-timing', legacyKey: null, label: '現在做還是再等一等', intent: 'timing_choice',
      assumptions: {}, spreads: ['timeline', 'three-time', 'horseshoe'],
      focus: ['dc-now-ready', 'dc-wait-gain', 'dc-wait-risk', 'dc-overlook', 'dc-priority'],
      placeholder: '例如：我不確定現在是不是好時機。',
      examples: { base: ['我不確定現在是不是好時機。'],
        byFocus: { 'dc-wait-risk': ['我怕再等下去機會就沒了。'] } } },
    { id: 'dc-stay', legacyKey: null, label: '我該留下還是離開', intent: 'stay_or_leave',
      assumptions: {}, spreads: ['fork', 'crosslove', 'celtic'],
      focus: ['dc-stay-why', 'dc-leave-ready', 'dc-a-cost', 'dc-overlook', 'dc-third'],
      placeholder: '例如：我一直在想要不要離開現在的環境。',
      examples: { base: ['我一直在想要不要離開現在的環境。'],
        byFocus: { 'dc-leave-ready': ['如果真的要走，我需要先準備什麼。'] } } },
    { id: 'dc-offer', legacyKey: null, label: '這個機會我該接受嗎', intent: 'accept_or_decline',
      assumptions: { specificOffer: true }, spreads: ['yesno', 'three-issue', 'fork'],
      focus: ['dc-accept-fit', 'dc-accept-risk', 'dc-do-cost', 'dc-not-do', 'dc-priority'],
      placeholder: '例如：有人給了我一個機會，但我不確定該不該接。',
      examples: { base: ['有人給了我一個機會，我不確定該不該接。'],
        byFocus: { 'dc-accept-risk': ['接下來之後最大的風險是什麼？'] } } },
    { id: 'dc-unclear', legacyKey: null, label: '我還不確定方向該怎麼想', intent: 'undecided_direction',
      assumptions: {}, spreads: ['celtic', 'three-mbs', 'horseshoe'],
      focus: ['dc-unclear-need', 'dc-unclear-signal', 'dc-priority', 'dc-overlook', 'dc-third'],
      placeholder: '例如：我知道現在的狀態不對，但說不出想要什麼。',
      examples: { base: ['我知道現在不太對，但說不出自己想要什麼。'],
        byFocus: { 'dc-unclear-need': ['我想先搞清楚自己現在最需要什麼。'] } } },
  ],
};
QUESTION_TAXONOMY.general = {
  categoryId: 'general', label: '綜合',
  placeholder: '例如：最近工作、關係和生活都在變動，我想知道該先處理什麼。',
  primaries: [
    { id: 'gn-now', legacyKey: 'overall-theme', label: '最近整體狀態如何', intent: 'overall_status',
      assumptions: {}, spreads: ['three-time', 'celtic', 'horseshoe'],
      focus: ['gn-theme', 'gn-mood', 'gn-action', 'gn-drain', 'gn-support'],
      placeholder: '例如：最近很多事情同時在變，我想先看看整體的狀態。',
      examples: { base: ['最近很多事同時在變，我想看看整體狀態。'],
        byFocus: { 'gn-drain': ['我覺得一直在耗，但說不出耗在哪裡。'] } } },
    { id: 'gn-first', legacyKey: 'priority-focus', label: '現在最需要先處理什麼', intent: 'top_priority',
      assumptions: {}, spreads: ['three-issue', 'horseshoe', 'single'],
      focus: ['gn-priority', 'gn-work', 'gn-relation', 'gn-money', 'gn-rest'],
      placeholder: '例如：工作、關係和生活都有事，我想知道先處理哪一個。',
      examples: { base: ['幾件事同時卡住，我不知道該先處理哪一個。'],
        byFocus: { 'gn-rest': ['我懷疑自己其實只是需要休息。'] } } },
    { id: 'gn-blind', legacyKey: 'hidden-blindspot', label: '我目前忽略了什麼', intent: 'blind_spot',
      assumptions: {}, spreads: ['celtic', 'three-issue', 'three-mbs'],
      focus: ['gn-blindspot', 'gn-drain', 'gn-relation', 'gn-mood', 'gn-support'],
      placeholder: '例如：事情一直卡著，我懷疑有什麼是我沒看到的。',
      examples: { base: ['事情一直卡著，我懷疑有什麼沒看到。'],
        byFocus: { 'gn-blindspot': ['我想知道自己一直繞過了什麼。'] } } },
    { id: 'gn-ahead', legacyKey: 'next-direction', label: '下一階段該往哪裡走', intent: 'next_direction',
      assumptions: {}, spreads: ['timeline', 'three-time', 'celtic'],
      focus: ['gn-next', 'gn-theme', 'gn-action', 'gn-work', 'gn-priority'],
      placeholder: '例如：目前的階段快結束了，我想知道下一步。',
      examples: { base: ['目前的階段快結束了，我想知道下一步。'],
        byFocus: { 'gn-next': ['我想知道接下來最值得投入什麼。'] } } },
  ],
};
